...
from meld_cmt_supernet import CrossModalSuperNet
from meld_nas import NASController

def collate_fn(batch):
    texts, audios, visuals, labels = zip(*batch)
    return pad_sequence(texts, batch_first=True), pad_sequence(audios, batch_first=True), pad_sequence(visuals, batch_first=True), torch.stack(labels)
...
def main(args):
    ...
    train_data = MELDDataset(split='train', data_dir=args.data_dir)
    dev_data = MELDDataset(split='dev', data_dir=args.data_dir)
    ...
    for round in range(args.nas_rounds):
        ...
        for epoch in range(args.epochs_per_round):
            ...
            if avg_val_acc > best_acc:
                ...
                torch.save(model.state_dict(), f"checkpoints/supernet_round{round + 1}.pth")
    ...
    if best_model_state_dict is not None:
        torch.save(best_model_state_dict, "checkpoints/best_supernet_overall.pth")
        with open("checkpoints/best_mask.json", "w") as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--prior_path", type=str, default="../meld_dataset/GAT/supernet_mask_prior.json")
