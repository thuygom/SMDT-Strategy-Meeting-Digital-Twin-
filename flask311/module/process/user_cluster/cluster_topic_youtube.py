    ...
    if filename.endswith('.xlsx'):
        ...
        try:
            ...
            if 'comment' not in df.columns or '감정' not in df.columns:
                print(f"⚠️ 'comment' 또는 '감정' 열이 없음: {filename}")
                continue

            # ✅ 이미 '주제'와 '군집' 열이 있고 모두 채워져 있으면 생략
            if '주제' in df.columns and '군집' in df.columns:
                if df['주제'].notna().all() and df['군집'].notna().all():
                    print(f"⏩ 이미 분류 완료된 파일: {filename}")
                    continue
            ...
            df.insert(6, '주제', topic_result)
            df.insert(7, '군집', cluster_result)
